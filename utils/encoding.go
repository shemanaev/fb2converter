...
// SrcEncoding defines supported source encoding
...
// DetectUTF attempts to detect encoding used in a buffer.
...
// DetectFileUTF attempts to detect encoding used in a file without changing file position.
func DetectFileUTF(file *os.File) (SrcEncoding, error) {
	...
	if ref, err := file.Seek(0, os.SEEK_SET); err != nil {
		return enc, err
	} else if ref != 0 {
		return enc, fmt.Errorf("unable to rewind file: %s", file.Name())
	}
	// read BOM
	buf := []byte{1, 1, 1, 1}
	...
	enc = DetectUTF(buf)
	// restore position
	if ref, err := file.Seek(offset, os.SEEK_SET); err != nil {
		return enc, err
	} else if ref != offset {
		return enc, fmt.Errorf("unable to reset file: %s", file.Name())
	}
	return enc, nil
}
